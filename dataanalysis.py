...
def XRDAnalysis(filePath: str) -> None:
    ...
    print(timeMarker)
    date = re.findall(r"(\d\d)\/(\d\d)\/(\d\d\d\d)|(\d\d\d\d)\/(\d\d)\/(\d\d)", timeMarker)[0]
    date = list(filter(None, date)) # 去空
    date = '/'.join(map(str, date))
    time = re.findall(r"(\d\d)\:(\d\d)\:(\d\d)", timeMarker)[0]
    ...
    plt.title("XRD Analysis " + date + " " + time)
    plt.show()
